...
def upload_subscriptions_to_delete(sub, is_activity, is_high_cost):
    try:
        sub = build_sub_object(sub, is_activity, is_high_cost)
        upload_to_table(table_name, sub)
    except:
        return "Error occured in the upload process"
    

def build_sub_object(sub, is_activity, is_high_cost):
