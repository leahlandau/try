# GITHUB_TOKEN=$1
# previous_release_created_at=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" "$API_PATH/releases/latest" | jq -r '.created_at')
# latest_tag=$(git describe --tags)
# previous_tag=$(git describe --tags $(git rev-list --tags --skip=1 --max-count=1))
# issues=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" "$API_PATH/issues?state=closed&per_page=100&since=$previous_release_created_at")
# issues_list=""
# while IFS= read -r row; do
#      number=$(echo "$row" | jq -r '.number')
#      title=$(echo "$row" | jq -r '.title')
#      assignees=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" "$API_PATH/issues/$number" | jq -r '.assignees[].login' | xargs)
...
# release_id=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" "$API_PATH/releases/tags/$latest_tag" | jq -r '.id')
